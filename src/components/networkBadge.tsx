...
export default function NetworkBadge({ className, variant = "full" }: NetworkBadgeProps) {
  ...
  const networkInfo = useMemo(() => {
    if (isHyperEVM) {
      return {
        ...
        imageSrc: "/built_on_hyperevm.png",
      };
    }
    return {
      name: "ETHEREUM",
    ...
    return (
      ...
      >
        <div className="relative w-2 h-2">
          <div className={cn("absolute inset-0 rounded-full", networkInfo.iconBg, "animate-pulse")} />
          <div className={cn("absolute inset-0 rounded-full", networkInfo.iconBg)} />
        </div>
