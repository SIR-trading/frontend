...
export function useUserLpPositions() {
  ...
  const { data: balance, isLoading: isLoadingBalance, refetch: refetchUserBalance } = useReadContract({
  ...
  const { data: stakerBalance, isLoading: isLoadingStakerBalance, refetch: refetchStakerBalance } = useReadContract({
    ...NonfungiblePositionManagerContract,
    functionName: 'balanceOf',
  ...
  }, [refetchUserBalance, refetchStakerBalance, refetchTokenIds, refetchPositions, refetchRewards]);
