...
function DeployOracleForm() {
  ...
  const handleDeploy = (e: React.FormEvent) => {
    ...
    if (!factoryAddress || !poolInitCodeHash) return;

    // Encode constructor arguments (address, bytes32)
    const encodedFactory = factoryAddress.slice(2).padStart(64, "0");
    const encodedHash = poolInitCodeHash.startsWith("0x")
      ? poolInitCodeHash.slice(2)
      : poolInitCodeHash;
    const deployData = ORACLE_BYTECODE + encodedFactory + encodedHash;
